...
| **代码检查**  | [references/codelinter-usage.md](references/codelinter-usage.md)                                       |
...
当用户要求“代码规范检查/质量门禁/CI 集成”时，优先使用 `codelinter`：

```
确认工程根目录 -> 读取或生成 code-linter.json5 -> 执行检查 -> 汇总问题并给出修复建议
```
...
建议默认策略：
1. 首次接入先使用 `recommended` 规则集，稳定后再扩展为 `all` 或按 `rules` 精细配置。
2. 在 `code-linter.json5` 中明确 `files` 和 `ignore`，避免扫描无关目录。
3. CI 中优先输出 `json`，便于做 error/warn 阈值判断与质量门禁。
...
当用户明确要求“落地改代码/可编译验证/质量检查”时，再执行以下流程：

1. 先执行 CodeLinter（如用户需要代码质量检查）：
   - `codelinter -c code-linter.json5 -f default .`
   - 需要报告时：`codelinter -c code-linter.json5 -f json -o reports/codelinter.json .`
2. 运行构建脚本进行验证：
   - macOS/Linux: `bash scripts/run.sh`
   - Windows: `.\scripts\run.ps1`
3. 失败时先分析错误并修复，再重试（建议最多 3 次）。
4. 若仍失败，请向用户汇报完整错误输出并请求下一步指示。
